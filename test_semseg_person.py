...
print("len(class2Label_dict)",len(class2Label_dict))

g_classes = ['ceiling','floor','wall','beam','column','window','door','table','chair','sofa','bookcase','board','clutter','person']
...
def main(args):
    ...
    experiment_dir = '/media/g111056119/SP PHD U3/datasets/log/pointnet/s3dis/' + args.log_dir
    ...
    NUM_CLASSES = len(class2Label_dict) # 14
    BATCH_SIZE = args.batch_size
    NUM_POINT = args.num_point

    # root = 'data/stanford_indoor3d_yun/'
    root = '/media/g111056119/SP PHD U3/datasets/S3DIS/npy_14classes/'
    ...
    with torch.no_grad():
        ...
        for batch_idx in range(num_batches):
            ...
            """
            total_seen_class_tmp[l]：統計整個場景中屬於類別 l 的真實標籤的總數。
            total_correct_class_tmp[l]：統計預測標籤為類別 l 且真實標籤也為類別 l 的總數，即正確預測的類別 l 的總數。
            total_iou_deno_class_tmp[l]：統計預測標籤為類別 l 或真實標籤為類別 l 的總數，即混合預測和真實標籤的類別 l 的總數。
            """
                    ...
                    fout_gt.write(
                        'v %f %f %f %d %d %d\n' % (
                            whole_scene_data[i, 0], whole_scene_data[i, 1], whole_scene_data[i, 2],  whole_scene_data[i, 3],
                             whole_scene_data[i, 4],  whole_scene_data[i, 5]))
            if args.visual:
                fout.close()
                fout_gt.close()
