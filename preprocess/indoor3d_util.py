"""
用於實驗 9類 Meta Train: S3DIS Meta Test: scanNet 同類下的泛化性
"""
import numpy as np
import glob
import os
...
g_classes = [x.rstrip() for x in open('/home/g111056119/Documents/7111056426/Reptile-Pytorch/datasets/classn_names_9_s3dis_scannet.txt')]
...
def collect_point_label(anno_path, out_filename, file_format='txt'):
    ...
    for f in glob.glob(os.path.join(anno_path, '*.txt')):
        ...
        points = np.loadtxt(f)
        labels = np.ones((points.shape[0],1)) * g_class2label[cls] # [label號碼] * 點數 array([[3.],[3.],[3.],....)
        points_list.append(np.concatenate([points, labels], 1)) # Nx7 XYZRGBL
